...
在微调过程中，您会看到类似下面的训练日志：

```
{'loss': 2.4172, 'grad_norm': 0.2653520107269287, 'learning_rate': 4e-05, 'epoch': 0.04}
{'loss': 2.4697, 'grad_norm': 0.2983250916004181, 'learning_rate': 8e-05, 'epoch': 0.08}
{'loss': 2.5866, 'grad_norm': 0.30050748586654663, 'learning_rate': 0.00012, 'epoch': 0.12}
{'loss': 2.508, 'grad_norm': 0.3454345464706421, 'learning_rate': 0.00016, 'epoch': 0.16}
{'loss': 2.5712, 'grad_norm': 0.4334571063518524, 'learning_rate': 0.0002, 'epoch': 0.2}
...
{'loss': 1.8455, 'grad_norm': 0.28270483016967773, 'learning_rate': 9.333333333333334e-05, 'epoch': 0.52}
```

**关键指标解读**：

- **loss**：损失值，越低越好。通常表示模型预测与目标之间的差距。
  - 在训练开始阶段，特别是在学习率预热期间，损失可能会波动或略微上升，这是正常现象
  - 随着训练进行，损失应该整体呈下降趋势（如上例中从2.5左右降至1.84）
  - 如果损失长时间停滞不下降，可能需要调整学习率或其他超参数

- **grad_norm**：梯度范数，表示参数更新的幅度。
  - 过大的梯度可能导致训练不稳定
  - 过小的梯度可能意味着学习效率低或陷入局部最小值
  - 通常在0.1-1.0之间是合理的
