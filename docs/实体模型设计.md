...
1. 使用泛型处理行项 ID
   - 不同系统可能使用不同类型的ID（String、Long、UUID等）
   - 使用泛型可以提供类型安全
...
我们可以根据具体的分配策略来命名分配服务实现类。比如之前提供的实现采用了贪心策略，可以命名为 `GreedyQuantityAllocationService`。

可以考虑支持业务人员“手动分配”，在此基础上系统进行自动分配。我们有必要区分这两种记录，比如使用一个 `IsAutoAllocated` 属性来区分。
