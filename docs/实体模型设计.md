...
* 工艺路线和工序（生产单的模板），用于简化批次信息的录入。
* 生产单（Production Run）和生产任务（工序的“实例”）。
* 生产任务使用的批次信息。
* 生产任务的产出批次信息。

考虑：

* 也许仍然可以在系统内保留 `WorkEffortInventoryAssign` 和 `InventoryItem` 实体，以及 `WorkEffortInventoryProduced` 实体，来实现原材料-成品的生产追溯。这有助于以后进行库存管理的扩展。
* 但是在界面上不呈现 `InventoryItem` 等库存相关的实体的状态。
* 对 WIP-成品的追溯，使用其他方式进行额外的处理？（即不使用 `InventoryItem` 和 `WorkEffortInventoryAssign` 等库存相关的实体，而是创建新的实体。）
