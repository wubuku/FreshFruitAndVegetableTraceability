...
* 现在将 GTIN(Global Trade Item Number|全球贸易项目代码) 作为一种“GoodIdentification”来实现。这个数据模型的约束是一个 GTIN 只允许关联到一种产品。虽然收货时允许使用临时的产品 Id 标识产品，但是之后在“补录” GTIN 时一个 GTIN 只允许关联到一种产品。
...
一个方便的说法是：这里的装运重量是“内包装的重量”，而装运箱类型中的重量是“外包装的重量”。
...
InventoryItem 是库存管理系统的核心实体，用于跟踪和管理单个库存单元。它详细记录了库存单元的所有关键信息，包括基本标识、数量、位置、状态、时间等维度的数据。
...
另外，对于时间相关类型，我们可能需要“舍弃”一定的精度后进行比较：
...
只要观察一下现有的库存模型（InventoryItem / InventoryItemDetail）设计，就可以看到光是 InventoryItem 已经有非常多”的属性（段）。
而这些属性很多都是 ID（也就是关联到的其他实体/表的“外键”），它们可以关联到各种实体。

另外，InventoryItem 虽然设计的粒度已经非常细——它的属性值的“所有可能的组合”的每一个都可以算是一个“库存账的子科目”。

但这些属性并不是每个都需要使用。
如果可能，管理库存的时候，没必要使用的“属性”其实都应该少给它们赋值。
因为这涉及“怎么归并库存数量”的问题。
在需要“使用”库存的时候，如果每条库存记录的“数量”都很小，那么可能本来“选择一条库存记录”就可以完成的一个操作，需要“选择多条库存记录”才能完成。

比如说，在入库的时候，如果对“接收的货物”填入了不同的 Description（这是一个备注性质的属性），因为程序不知道这个“描述”有什么“妙用”（也许它指示了这批货物非常重要的“出库”规则），那么在出库的时候，程序只能强制用户选择到“每一条描述不同的库存记录”，而不应自作聪明地“自动匹配”。
这都会对用户的使用体验有影响。


#### Raw material 

“原始需求”描述，认为原材料需要记录的“detail level”属性，以及我们的分析：

- lot。这应该是 InventoryItem 的属性。
- vendor。这应该是 InventoryItem 的 Product Id 属性间接关联的属性。
- quantity。这应该是 InventoryItem 的属性。InventoryItemDetail 的数量属性是 **quantityDiff**。
- received date。这可以是 InventoryItem 的属性（`datetimeReceived`）；但有些情况下，不在 InventoryItem 中记录这个属性（让它为空），而是考虑仅通过 InventoryItemDetail 来追溯知道库存“怎么来的”可能会更灵活。
- storage location。这应该是 InventoryItem 的属性。
- reference to receiving record。这应该是 InventoryItemDetail 的相关属性（`receiptId`）**间接关联**的信息。

#### WIPs 

“原始需求”描述，认为 WIP 需记录的 detail level 属性，以及我们的分析：

- lot。同上。应该是 InventoryItem 的属性。
- production date。这可以是 InventoryItem 的属性（`datetimeManufactured`）；也有些情况下，可能不在 InventoryItem 中记录这个属性（让它为空），而是通过 InventoryItemDetail 来追溯知道库存是“那个工序运行”的结果。
- quantity。同上。
- storage location。同上。

#### Finished product

“原始需求”描述，认为成品需记录的 detail level 属性，以及我们的分析：

- lot。同上。应该是 InventoryItem 的属性。
- production date。同上。
- quantity。同上。
- storage location。同上。
- customer。这应该是通过 InventoryItemDetail 的 `orderId` 等属性**间接关联**的信息。
- reference to SO。这应该是 InventoryItemDetail 的属性（`orderId` 等）**间接关联**的信息。

#### 对库存变化进行记录的要求

原始需求描述：

- showcase a log of historical inventory adjustments
  - allow filtering based on type of item or warehouse

- Types of adjustments
  - pick to shop floor: move items from inventory to shop floor for production
  - scrap: remove items from inventory 
  - return to vendor: track received items that are rejected and returned to vendor 
  - return to cooler: return items from shop floor to storage
  - location adjustment: adjustment locations of items in warehouses
  - correction: adjustment quantity to correct mistakes from recording

严格来说，InventoryItemDetail（条目）本质上就是 log of historical inventory item changes（账目数量变化的日志）。

- Shop floor 库存数量的变化（非意外情况），主要来自于“生产”的领料和产出。所以，InventoryItemDetail 需要和 WorkEffort（工作项） 相关联。
- Return 到 Vendor 引起的数量变化，需要和“退货单”关联（退货单实体，叫做 Return Header，或者直接说 Return）。所以我们可以看到 InventoryItemDetail 中，有 `returnId` 等字段。
- Return to cooler，location adjustment，库存的货品本身没有发生“质变”、数量也没有变化，正数和负数的条目需要成对记录（绝对值相同的一对 quantityDiff，一个为正数，一个为负数）。可以考虑使用 `InventoryTransfer` 实体来记录，然后让 `InventoryItemDetail` 关联到 `InventoryTransfer` 实体。（下文有讨论 `InventoryTransfer` 实体。）
- 其他意外情况（包括报废），一般是通过“实物盘点”（PhysicalInventory）来纠正。你可以认为它就是上面说的 Correction。（在实际编码实现的时候，可能有时候会考虑把“盘点单”的创建和执行隐藏在“背后”，用户可能感觉不到这个概念。）


说到“报废”，这里讨论一下“报废率”的问题：
- 一般在 BOM 中，在记录一个标准单位成品所需要的原材料的标准消耗数量之外，可以记录一个报废因子（scrap factor，一般是一个百分数）。可以认为这是一个“标准报废率”。它代表在生产过程中预期会被浪费或报废的物料百分比。
- 生产领料的时候可能会考虑到这个因素，直接第一次就会多领用一些。这时候，其实“报废”已经被合并到生产领料出库的操作中了。
- 如果生产对原料的使用情况和预期不符，可能还会通过各种方式再次领料补充。
- 事后统计单位成品实际投入的原材料，和标准消耗量之间的差异，可以得到实际“报废率”。


### 对比 WMS 的 InventoryItemEntry 实体
...
InventoryItemDetail 实体中没有 `inventoryTransferId` 字段，这可能会导致库存移动操作的追溯性问题。

追溯性问题分析：

在 OFBiz 的标准设计中，这种情况确实存在一些局限性：
- InventoryItemDetail 记录了库存数量的变化，但没有直接关联到触发这些变化的 InventoryItemTransfer 记录
- 当看到两条对应的 InventoryItemDetail 记录（一条负值一条正值）时，很难直接确定它们是否来自同一个库存移动操作

OFBiz 的解决方式：

OFBiz 可能可以尝试通过以下几种方式解决这个追溯问题，但它们都不是十分完美：
- 通过描述字段 - description 字段可能会包含库存转移的相关信息
- 通过原因枚举 - reasonEnumId 字段可能会标识这是一个库存转移操作
- 时间匹配 - 系统服务可能通过相近的 effectiveDate 来关联相关记录
- 服务层面的处理 - 在应用程序逻辑层面维护这种关联

改进考虑：扩展 InventoryItemDetail 实体，添加 `inventoryTransferId` 字段，并关联 InventoryTransfer 实体。


## WorkOrderItemFulfillment 实体

OFBiz 的 WorkOrderItemFulfillment 实体定义：
...
这个实体存储了哪个工作任务(WorkEffort)用于满足哪个订单项。


## Container 相关实体

OFBiz 的 Container 相关实体定义：

```xml
    <entity entity-name="Container" package-name="org.apache.ofbiz.product.facility" title="Container">
        <field name="containerId" type="id"></field>
    ...
    <entity entity-name="ContainerType" package-name="org.apache.ofbiz.product.facility" title="Container Type">
        <field name="containerTypeId" type="id"></field>
        <field name="description" type="description"></field>
...
> 这类 UOM ID 使用特定的前缀？或者没有必要？比如 `AI-3100` vs `3100`。
> 但不管怎么说，`uomTypeId` 字段应该明确标识为 GS1 Application Identifiers。
...
## Transformation（生产）追溯实现方案

### CTE 表格分析

我们可以从最终需要形成的 Transformation CTE 表格来分析（倒推）一个最简实现方案：
...
最简实现方案的用户日常作业操作体验大致是：

1. 创建产品的生产单
   - 选择产品。
   - 选择工艺路线。如果不选择，则使用默认的工艺路线。
   - 系统根据用户的选择，查找 BOM 信息，创建生产单和生产任务。
2. 在每道工序（生产任务）中，输入（选择）使用材料的批次信息；
3. 在完成每道工序（生产任务）时，输入产出品的批次信息。

> 可以结合“为了完成上面的用户体验”，考虑需要使用以及如何剪裁文档 `docs/OFBiz_制造模块分析.md` 中的实体。

### 实体模型设计考虑

实现方案可能需要的实体：
- 原料-成品关联，也就是 BOM（Bill of Materials）。
- 工艺路线和工序（生产单的模板），用于简化后续的追溯信息的录入。
- 生产单（Production Run）和生产任务（Production Run Task，或者说“工序的实例”）。
- 生产任务使用的批次信息。这里可能涉及不止一个实体（表），而是类似 `WorkEffortInventoryAssign`、`InventoryItem` 这样的多个实体。
- 生产任务的产出批次信息。同样，可能涉及不止一个实体（表），而是类似 `WorkEffortInventoryProduced`、`InventoryItem` 这样的多个实体。

考虑：
- 也许仍然可以在系统内保留 `WorkEffortInventoryAssign`、`InventoryItem`，以及 `WorkEffortInventoryProduced` 实体，来实现原材料-成品的生产追溯。
- 系统先不维护“库存数量”的正确性，但是使用这些实体可以实现“批次”的选择，这有助于以后进行库存管理的扩展。
- 因此在界面上先不去呈现 `InventoryItem` 等库存相关实体的状态（这里主要是指“库存数量”）。

### 生产追溯的完整性保障

为了形成完整的生产追溯信息，考虑：
- 使用材料的批次只能选择，不能输入（临时创建），否则追溯可能“断链”。批次只能来自于 Receiving 或者之前的生产单的产出。
- 产出品的批次，考虑使用可读性良好的编码规则赋予一个“唯一的批次号”，方便在其他工序中选择（引用）。
  * 虽然系统可能还会自动生成一个唯一的“代理键”，但是这个代理键可读性较差，可能并不方便用户进行识别和选择。 
  * 这个批次号，考虑自动生成。（允许用户输入/修改？但是需要符合规则，并保证唯一性）
  * 这里的“唯一批次号”，唯一性只需要在“一定范围内”是唯一的即可。需要考虑好唯一性规则，比如在“当前设施”内唯一，只能被唯一的“生产任务（工序的实例）”使用。

所以，如果按照上面的设想来做设计，焦点似乎就是：如何让“使用批次的选择”更方便以及不容易出错。
- 选择来自于 Receiving 的原材料时，按照先进先出的原则，优先选择较早的批次。
- 但是又不能让所有历史存在的批次都提供给用户。（太早的批次可能就不应该显示了。）
  * 考虑 WIP 的可选批次的只能选择一定时间范围内的批次。
  * 非 WIP 的产品，我们使用“库存数量评估”的方式来确定显示的优先级。比如系统评估某个原材料（产品）大概率已经完全领用，那么就不用显示在比较靠前的位置。（上面说过，我们可以考虑保存库存相关实体，那实际上系统内部可以保存一个可能不太准确的数量，来帮助我们确定显示的优先级。）
  * 提供“使用批次”的维护界面，强制让某些批次不再显示？

### 产品的保质期管理

还可以考虑给产品增加两个属性，过了“保质期”的批次在选择时就不再显示（只考虑对 WIP 产品如此？）：
- `defaultShelfLife`：Default shelf life value
- `defaultShelfLifeUomId`：Unit of measure for shelf life (hours, days, months, etc)
...
见：
- `data/demo/UomDemoData.xml`
- `data/demo/ManufacturingDemoData.xml`
- `data/demo/LotTracingTestData.xml`


#### 测试结果

按照 `LotTracingTestData.xml` 中的数据，执行集成测试来打印“成品-原材料”以及“原材料-成品”的追溯树。
...
以下信息来自集成测试 `LotTracingServiceTest.java` 的打印结果。

> 打印格式：`<批次 ID> (<产品 ID>) [<生产任务 ID>]`。
> 如果你只关心“产品 / 批次”，那么忽略“生产任务 ID”对追溯结果进行归并即可。

从成品巧克力蛋糕批次追溯使用的原材料批次：

```text
=== Input Tree for Chocolate Cake ===
└── CHOC_CAKE_LOT_001 (CHOC_CAKE_STD)
    ├── CAKE_BASE_LOT_001 (WIP_CAKE_BASE) [PROD_RUN_04]
    │   ├── EGG_LOT_001 (RAW_EGG) [PROD_RUN_01]
    │   ├── FLOUR_LOT_001 (RAW_FLOUR) [PROD_RUN_01]
    │   ├── MILK_LOT_001 (RAW_MILK) [PROD_RUN_01]
    │   └── SUGAR_LOT_001 (RAW_SUGAR) [PROD_RUN_01]
    ├── CHOC_SAUCE_LOT_001 (WIP_CHOC_SAUCE) [PROD_RUN_04]
    │   ├── CHOC_LOT_001 (RAW_CHOC) [PROD_RUN_03]
    │   └── MILK_LOT_001 (RAW_MILK) [PROD_RUN_03]
    └── CREAM_FILL_LOT_001 (WIP_CREAM_FILL) [PROD_RUN_04]
        ├── CREAM_LOT_001 (RAW_CREAM) [PROD_RUN_02]
        └── SUGAR_LOT_001 (RAW_SUGAR) [PROD_RUN_02]
```

从原材料面粉批次追溯生产的产品（直到成品巧克力蛋糕）批次：

```text
=== Output Tree for Flour ===
└── FLOUR_LOT_001 (RAW_FLOUR)
    └── CAKE_BASE_LOT_001 (WIP_CAKE_BASE) [PROD_RUN_01]
        └── CHOC_CAKE_LOT_001 (CHOC_CAKE_STD) [PROD_RUN_04]
```

从原材料牛奶批次追溯生产的产品（直到成品巧克力蛋糕）批次：

```text
=== Output Tree for Milk ===
└── MILK_LOT_001 (RAW_MILK)
    ├── CAKE_BASE_LOT_001 (WIP_CAKE_BASE) [PROD_RUN_01]
    │   └── CHOC_CAKE_LOT_001 (CHOC_CAKE_STD) [PROD_RUN_04]
    └── CHOC_SAUCE_LOT_001 (WIP_CHOC_SAUCE) [PROD_RUN_03]
        └── CHOC_CAKE_LOT_001 (CHOC_CAKE_STD) [PROD_RUN_04]
```

这个追溯树显示:
1. MILK_LOT_001 被用于生产两个半成品:
   - CAKE_BASE_LOT_001 (在 PROD_RUN_01 工序中)
   - CHOC_SAUCE_LOT_001 (在 PROD_RUN_03 工序中)
2. 这两个半成品最终都被用于生产同一个成品:
   - CHOC_CAKE_LOT_001 (在 PROD_RUN_04 工序中)
...
比如，在测试数据中，我们有部分数据基于这样的假设来创建：

- WIP 批次使用日期作为批次号: WIP_20240102。所有中间工序的产出都使用这个批次号。
- 成品批次使用日期作为批次号: FG_20240102。所有成品都使用这个批次号。
- WIP 批次不设置数量和过期时间。

那么，从原材料面粉批次追溯生产的产品（直到成品蛋糕）批次：
...
└── MILK_LOT_001 (RAW_MILK)
    ├── CAKE_BASE_LOT_001 (WIP_CAKE_BASE) [PROD_RUN_01]
    │   └── CHOC_CAKE_LOT_001 (CHOC_CAKE_STD) [PROD_RUN_04]
    ├── WIP_20240102 (WIP_CAKE_BASE) [PROD_RUN_05]
    │   └── FG_20240102 (CHOC_CAKE_STD) [PROD_RUN_06]
    └── CHOC_SAUCE_LOT_001 (WIP_CHOC_SAUCE) [PROD_RUN_03]
        └── CHOC_CAKE_LOT_001 (CHOC_CAKE_STD) [PROD_RUN_04]
```
...
### MergePatch 命令的 `IsPropertyXxxRemoved` 属性

#### 问题背景

在处理对象的部分更新（PATCH）请求时，假设这个请求使用 JSON 格式序列化，我们经常遇到一个问题：如何区分以下两种情况？

1. 客户端想要将某个属性设置为 null
2. 客户端不想修改某个属性(在 JSON 中不包含该属性)
...
大多数 JSON 反序列化库在处理这两种情况时存在局限性：

1. 当 JSON 对象中某个属性的值为 null 时，反序列化后的 Java 对象中对应属性也为 null
2. 当 JSON 对象中不包含某个属性时，反序列化后的 Java 对象中对应属性也为 null

这导致在 Java 层面无法区分“属性值被设置为 null”和“属性未被包含在请求中”这两种情况。

#### 解决方案

为了明确区分这两种情况，我们引入了 `IsPropertyXxxRemoved` 布尔标记：
...
1. 明确表达意图：通过显式的布尔标记表明“要删除这个属性”的意图
2. 避免歧义：可以清晰区分“要设置为 null”和“不修改”这两种情况
3. 向后兼容：不影响现有的 JSON 处理逻辑

#### 使用示例

```json
// 示例 1: 明确表示要删除 description 属性
{
  "description": null,
  "isPropertyDescriptionRemoved": true
}

// 示例 2: 不修改 description 属性
{
  "title": "新标题"
}

// 示例 3: 将 description 设置为新值
{
  "description": "新描述"
}
```
...
1. 命名规范：使用 `isPropertyXxxRemoved` 的格式，其中 Xxx 是要删除的属性名
2. 文档说明：在 API 文档中明确说明这些标记的用途
3. 验证逻辑：确保当 `isPropertyXxxRemoved` 为 true 时，对应的属性值必须为 null
