...
* 现在将 GTIN 作为一种“GoodIdentification”来实现。这个数据模型的约束是一个 GTIN 只允许关联到一种产品。虽然收货时允许使用临时的产品 Id 标识产品，但是之后在“补录” GTIN 时一个 GTIN 只允许关联到一种产品。
...
#### CTE 的 TLC Source or TLC Source Reference
