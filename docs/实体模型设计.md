...
- `inventoryItemTypeId`: 库存类型标识（如：成品、原材料、在制品等）
- `productId`: 关联的产品标识
...
有一点需要考虑，就是不同类型的“相等”比较方法可能不同。

比如：OffsetDateTime 的 `equals()` 方法是基于 local date-time 和 offset 来比较的，也就是说会同时比较本地时间和时区偏移量。如果要比较两个时间是否表示时间线上的同一时刻，应该使用 `isEqual()` 方法。在 Java time 包中的 `ZonedDateTime` 和 `OffsetTime` 也是类似的情况。
...
- 对 WIP-成品的追溯，使用其他方式进行额外的处理？
  * 即不使用 `InventoryItem`、`WorkEffortInventoryAssign` 等库存相关的实体，而是创建“其他实体”。
  * 但是这些实体可能和`InventoryItem`、`WorkEffortInventoryAssign` 等实体有一定的类似之处。
...
还可以考虑给 WIP 产品增加两个属性，过了“保质期”的批次在选择时就不再显示：
