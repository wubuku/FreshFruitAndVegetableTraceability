...
public class BffSupplierApplicationServiceImpl implements BffSupplierApplicationService {
    ...
    private void enrichFacilityDetails(BffSupplierDto dto, String supplierId) {
        List<BffFacilityProjection> facilityProjections = bffFacilityRepository
                .findFacilitiesByOwnerPartyId(supplierId);
    ...
    public String when(BffSupplierServiceCommands.CreateSupplier c) {
        ...
        if (c.getSupplier().getBusinessContacts() != null && !c.getSupplier().getBusinessContacts().isEmpty()) {
            String contactMechId = bffBusinessContactService
                    .createMiscContact(c.getSupplier().getBusinessContacts().get(0), c);
    ...
    public void when(BffSupplierServiceCommands.UpdateSupplier c) {
        ...
        if (bffSupplier.getInternalId() != null && !bffSupplier.getInternalId().isBlank()) {
            ...
            String partyId = bffSupplierRepository.queryByPartyIdentificationTypeIdAndIdValue(
                    PARTY_IDENTIFICATION_TYPE_INTERNAL_ID, bffSupplier.getInternalId());
            if (partyId != null && !partyId.equals(supplierId)) {
                throw new IllegalArgumentException(String.format(
                        "Vendor Number:%s is already in use. Please try a different one.",
                        bffSupplier.getInternalId()));
            }
        }
        ...
        if (partyRole != null) {// 有就改
            ...
            simpleMergePatchPartyRole
                    .setSupplierProductTypeDescription(bffSupplier.getSupplierProductTypeDescription());
        ...
        List<String> newFacilityIds = bffSupplier.getFacilities().stream().map(BffFacilityDto::getFacilityId)
    ...
    private String createSupplierParty(BffSupplierDto supplier, Command c) {
        ...
        if (supplier.getSupplierId() != null) {
            ...
            if (partyState != null) {
                throw new IllegalArgumentException(
                        String.format(ERROR_SUPPLIER_ALREADY_EXISTS, supplier.getSupplierId()));
            }
        ...
        createParty.setPreferredCurrencyUomId(
                supplier.getPreferredCurrencyUomId() != null ? supplier.getPreferredCurrencyUomId()
        ...
        if (supplier.getInternalId() != null && !supplier.getInternalId().isBlank()) {
            ...
            if (bffSupplierRepository.countByPartyIdentificationTypeIdAndIdValue(PARTY_IDENTIFICATION_TYPE_INTERNAL_ID,
                    supplier.getInternalId()) > 0) {
                throw new IllegalArgumentException(String.format(
                        "Vendor Number:%s is already in use. Please try a different one.", supplier.getInternalId()));
            }
    ...
    public void when(BffSupplierServiceCommands.BatchAddSuppliers c) {
        ...
        for (var supplier : c.getSuppliers()) {
            if (supplier.getSupplierId() != null && !supplier.getSupplierId().isEmpty()) {
                if (supplierIds.contains(supplier.getSupplierId())) {
                    throw new IllegalArgumentException(
                            String.format(ERROR_DUPLICATE_SUPPLIER, supplier.getSupplierId()));
                }
        ...
        for (BffSupplierDto supplierDto : c.getSuppliers()) {
            ...
            if (supplierDto.getBusinessContacts() != null && !supplierDto.getBusinessContacts().isEmpty()) {
                String contactMechId = bffBusinessContactService
                        .createMiscContact(supplierDto.getBusinessContacts().get(0), c);
                createPartyContactMechAssociation(partyId, contactMechId, "-PE", c);
            }
            // 设施
