...
@Repository
public class HibernateFacilityStateQueryRepository implements FacilityStateQueryRepository {
    @PersistenceContext
    ...
    public FacilityState get(String id) {
        FacilityState state = (FacilityState)getEntityManager().find(AbstractFacilityState.SimpleFacilityState.class, id);
        return state;
    }
    ...
    public Iterable<FacilityState> getAll(Integer firstResult, Integer maxResults) {
        ...
        CriteriaQuery<AbstractFacilityState.SimpleFacilityState> cq = cb.createQuery(AbstractFacilityState.SimpleFacilityState.class);
        Root<AbstractFacilityState.SimpleFacilityState> root = cq.from(AbstractFacilityState.SimpleFacilityState.class);
        cq.select(root);
        addNotDeletedRestriction(cb, cq, root);
        TypedQuery<AbstractFacilityState.SimpleFacilityState> query = em.createQuery(cq);
        JpaUtils.applyPagination(query, firstResult, maxResults);
        return query.getResultList().stream().map(FacilityState.class::cast).collect(Collectors.toList());
    }
    ...
    @Transactional(readOnly = true)
    public Iterable<FacilityState> get(org.dddml.support.criterion.Criterion filter, List<String> orders, Integer firstResult, Integer maxResults) {
        EntityManager em = getEntityManager();
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<AbstractFacilityState.SimpleFacilityState> cq = cb.createQuery(AbstractFacilityState.SimpleFacilityState.class);
        Root<AbstractFacilityState.SimpleFacilityState> root = cq.from(AbstractFacilityState.SimpleFacilityState.class);
    ...
    @Transactional(readOnly = true)
    public FacilityState getFirst(Iterable<Map.Entry<String, Object>> filter, List<String> orders) {
        List<FacilityState> list = (List<FacilityState>)get(filter, orders, 0, 1);
    ...
    @Transactional(readOnly = true)
    public FacilityState getFirst(Map.Entry<String, Object> keyValue, List<String> orders) {
    ...
    protected void addNotDeletedRestriction(CriteriaBuilder cb, CriteriaQuery<?> cq, Root<AbstractFacilityState.SimpleFacilityState> root) {
