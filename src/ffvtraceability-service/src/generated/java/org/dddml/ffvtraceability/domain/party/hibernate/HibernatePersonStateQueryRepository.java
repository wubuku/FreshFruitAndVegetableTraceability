...
public class HibernatePersonStateQueryRepository implements PersonStateQueryRepository {
    ...
    public PersonState get(String id) {
        PersonState state = (PersonState)getEntityManager().find(AbstractPersonState.SimplePersonState.class, id);
        return state;
    }
    ...
    public Iterable<PersonState> getAll(Integer firstResult, Integer maxResults) {
        ...
        CriteriaQuery<AbstractPersonState.SimplePersonState> cq = cb.createQuery(AbstractPersonState.SimplePersonState.class);
        Root<AbstractPersonState.SimplePersonState> root = cq.from(AbstractPersonState.SimplePersonState.class);
        cq.select(root);
        addNotDeletedRestriction(cb, cq, root);
        TypedQuery<AbstractPersonState.SimplePersonState> query = em.createQuery(cq);
        JpaUtils.applyPagination(query, firstResult, maxResults);
        return query.getResultList().stream().map(PersonState.class::cast).collect(Collectors.toList());
    }
