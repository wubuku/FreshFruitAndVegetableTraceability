...
@Repository("physicalInventoryStateRepository")
public class HibernatePhysicalInventoryStateRepository implements PhysicalInventoryStateRepository {
    ...
    public PhysicalInventoryState get(String id, boolean nullAllowed) {
        PhysicalInventoryState.SqlPhysicalInventoryState state = (PhysicalInventoryState.SqlPhysicalInventoryState)getEntityManager().find(AbstractPhysicalInventoryState.SimplePhysicalInventoryState.class, id);
        if (!nullAllowed && state == null) {
            state = new AbstractPhysicalInventoryState.SimplePhysicalInventoryState();
            state.setPhysicalInventoryId(id);
        }
    ...
    public void merge(PhysicalInventoryState detached) {
        PhysicalInventoryState persistent = getEntityManager().find(AbstractPhysicalInventoryState.SimplePhysicalInventoryState.class, detached.getPhysicalInventoryId());
        if (persistent != null) {
    ...
    private void merge(PhysicalInventoryState persistent, PhysicalInventoryState detached) {
        ((AbstractPhysicalInventoryState) persistent).merge(detached);
    }
