...
public abstract class AbstractRoleTypeApplicationService implements RoleTypeApplicationService {
    ...
    public AbstractRoleTypeApplicationService(RoleTypeStateRepository stateRepository, RoleTypeStateQueryRepository stateQueryRepository) {
    ...
    public void when(RoleTypeCommand.CreateRoleType c) {
        ...
        RoleTypeState.SqlRoleTypeState ss = ((RoleTypeState.SqlRoleTypeState)s);
    ...
    protected void update(RoleTypeCommand c, Consumer<RoleTypeState> action) {
        ...
        RoleTypeCommand.throwOnInvalidStateTransition(state, c);
        action.accept(state);
        persist(eventStoreAggregateId, c.getVersion() == null ? RoleTypeState.VERSION_NULL : c.getVersion(), state); // State version may be null!

    }

    private void persist(EventStoreAggregateId eventStoreAggregateId, long version, RoleTypeState state) {
        getStateRepository().save(state);
    }

    protected boolean isDuplicateCommand(RoleTypeCommand command, EventStoreAggregateId eventStoreAggregateId, RoleTypeState state) {
    ...
    protected static void throwOnConcurrencyConflict(RoleTypeState s, RoleTypeCommand c) {
        Long stateVersion = s.getVersion();
        Long commandVersion = c.getVersion();
        if (commandVersion == null) { commandVersion = RoleTypeState.VERSION_NULL; }
        if (!(stateVersion == null && commandVersion.equals(RoleTypeState.VERSION_NULL)) && !commandVersion.equals(stateVersion)) {
    ...
    public static class SimpleRoleTypeApplicationService extends AbstractRoleTypeApplicationService {
        public SimpleRoleTypeApplicationService(RoleTypeStateRepository stateRepository, RoleTypeStateQueryRepository stateQueryRepository)
        {
