...
public class GeoResource {
    ...
    public String post(@RequestBody CreateOrMergePatchGeoDto.CreateGeoDto value,  HttpServletResponse response) {
        try {
            GeoCommand.CreateGeo cmd = value;//.toCreateGeo();
            if (cmd.getGeoId() == null) {
                throw DomainError.named("nullId", "Aggregate Id in cmd is null, aggregate name: %1$s.", "Geo");
            }
            String idObj = cmd.getGeoId();
            cmd.setRequesterId(SecurityContextUtil.getRequesterId());
            geoApplicationService.when(cmd);
     ...
     */
    @PutMapping("{geoId}")
    public void put(@PathVariable("geoId") String geoId, @RequestBody CreateOrMergePatchGeoDto value) {
        try {
            if (value.getVersion() != null) {
                value.setCommandType(Command.COMMAND_TYPE_MERGE_PATCH);
                GeoCommand.MergePatchGeo cmd = (GeoCommand.MergePatchGeo) value.toSubclass();
                GeoResourceUtils.setNullIdOrThrowOnInconsistentIds(geoId, cmd);
                cmd.setRequesterId(SecurityContextUtil.getRequesterId());
                geoApplicationService.when(cmd);
                return;
            }
            ...
            GeoCommand.CreateGeo cmd = (GeoCommand.CreateGeo) value.toSubclass();
            GeoResourceUtils.setNullIdOrThrowOnInconsistentIds(geoId, cmd);
            cmd.setRequesterId(SecurityContextUtil.getRequesterId());
            geoApplicationService.when(cmd);
     ...
     */
    @PatchMapping("{geoId}")
    public void patch(@PathVariable("geoId") String geoId, @RequestBody CreateOrMergePatchGeoDto.MergePatchGeoDto value) {
        try {

            GeoCommand.MergePatchGeo cmd = value;//.toMergePatchGeo();
            GeoResourceUtils.setNullIdOrThrowOnInconsistentIds(geoId, cmd);
            cmd.setRequesterId(SecurityContextUtil.getRequesterId());
            geoApplicationService.when(cmd);
